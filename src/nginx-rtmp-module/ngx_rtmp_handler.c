...
ngx_rtmp_ping(ngx_event_t *pev)
{
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                  "ping flags: %d, %d, %d", s->ping_reset, s->ping_active, s->relay);
    /*if (s->ping_reset) {
    	if (s->ping_active) {
	...
	if (s->ping_active) {
		if (s->relay == 0) {
			ngx_log_error(NGX_LOG_INFO, c->log, 0,
						"[ping_client_unresponded] ping: unresponded");
		} else {

			ngx_log_error(NGX_LOG_INFO, c->log, 0,
						"[ping_cdn_unresponded] ping: unresponded");
		}
    ...
    for( ;; ) {
        ...
        if (b->pos == b->start) {
            ...
            if (csid >= (uint32_t)cscf->max_streams) {
                ngx_log_error(NGX_LOG_INFO, c->log, 0,
                    "[too_big_csid] RTMP in chunk stream too big: %D >= %D",
            ...
            if (h->mlen > cscf->max_message) {
                ngx_log_error(NGX_LOG_INFO, c->log, 0,
                        "[too_big_message] too big message: %uz", cscf->max_message);
                ngx_rtmp_finalize_session(s);
                return;
            }
        }
        ...
        ngx_rtmp_header_t *lh, ngx_chain_t *out)
{
    ...
    if (h->csid >= (uint32_t)cscf->max_streams) {
        ngx_log_error(NGX_LOG_INFO, c->log, 0,
                "[too_big_csid] RTMP out chunk stream too big: %D >= %D",
