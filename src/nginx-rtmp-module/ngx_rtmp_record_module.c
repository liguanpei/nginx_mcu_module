...
static void *
ngx_rtmp_record_create_main_conf(ngx_conf_t *cf);
...
static ngx_command_t  ngx_rtmp_record_commands[] = {
	...
	{ ngx_string("queue"),
	  ...
	  offsetof(ngx_rtmp_record_main_conf_t, queue),
	  NULL },
...
static ngx_rtmp_module_t  ngx_rtmp_record_module_ctx = {
    ...
    ngx_rtmp_record_create_main_conf,       /* create main configuration */
...
ngx_rtmp_record_create_main_conf(ngx_conf_t *cf)
{
	ngx_rtmp_record_main_conf_t *rmcf;

	rmcf = ngx_pcalloc(cf->pool, sizeof(ngx_rtmp_record_main_conf_t));
	if (rmcf == NULL) {
		return NULL;
	}

	rmcf->queue = ngx_pcalloc(cf->pool, sizeof(ngx_queue_t));
	ngx_queue_init(rmcf->queue);

	ngx_rtmp_record_main_conf = rmcf;
...
ngx_int_t
ngx_rtmp_arg(ngx_str_t src, u_char *name, size_t len, ngx_str_t *value)
{
    u_char  *p, *last;
                          ...
                          ngx_rtmp_record_rec_ctx_t *rctx, ngx_str_t *path)
{
	...
	int                             has_liveid_flag = 0;

    ctx = ngx_rtmp_get_module_ctx(s, ngx_rtmp_record_module);
	rmcf = ngx_rtmp_get_module_main_conf(s, ngx_rtmp_record_module);
    ...
    *p++ = '/';
    ...
    /* append timestamp */
	/* add by ligp */
	/* if liveid is given, we make the path with liveid, otherwise with tmestamp */
	if (ngx_strlen(ctx->liveid) != 0 || ngx_queue_get_liveid(rmcf->queue, ctx->name, liveid) == 1) {
		has_liveid_flag = 1;
	}
    if (rracf->unique) {
		if (has_liveid_flag == 1) {
        	p = ngx_cpymem(p, "-", ngx_min(1, l - p));
			if (ngx_strlen(ctx->liveid) != 0) {
        		p = ngx_cpymem(p, ctx->liveid, ngx_min((signed)(ngx_strlen(ctx->liveid)), l - p));
			} else {
        		p = ngx_cpymem(p, liveid, ngx_min((signed)(ngx_strlen(liveid)), l - p));
			}
		} else {
    ...
    if (rctx->file.fd == NGX_INVALID_FILE) {
        ...
        if (err != NGX_ENOENT) {
            ngx_log_error(NGX_LOG_CRIT, s->connection->log, err,
                          "[record_file_open_failed] record: %V failed to open file '%V'",
                          &rracf->id, &path);
        }
...
ngx_rtmp_record_publish(ngx_rtmp_session_t *s, ngx_rtmp_publish_t *v)
{
	...
	ngx_str_t 						tmp_args;
    u_char                         *p;
	ngx_str_t 						liveid = ngx_null_string;
	...
	/* add by ligp */
	/* we init liveid here add save it to ctx */
	ngx_str_set(&tmp_args, v->args);
	tmp_args.len = ngx_strlen(v->args);
	if (ngx_rtmp_arg(tmp_args, (u_char *) "liveid", sizeof("liveid") - 1, &liveid) != NGX_OK) {
    	ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
				"record: liveid is empty, args: %s",
				ctx->args);

        if (ngx_find_queue(ngx_rtmp_record_main_conf->queue, ctx->name) == 0) {
            ...
            ngx_cpystrn(tmp_info->stream_name, ctx->name, NGX_RTMP_MAX_NAME);
    ...
    if (liveid.len > 0) {
        ngx_memcpy(ctx->liveid, liveid.data, sizeof(ctx->liveid));
    }
    /* end */
...
ngx_rtmp_record_node_av(ngx_rtmp_session_t *s, ngx_rtmp_record_rec_ctx_t *rctx, ngx_rtmp_header_t *h, ngx_chain_t *in)
{
    ...
    if (brkframe && (rracf->flags & NGX_RTMP_RECORD_MANUAL) == 0) { //没配manual模式
    ...
    if ((rracf->flags & NGX_RTMP_RECORD_MANUAL) && 
            (ngx_queue_need_record(rmcf->queue, ctx->name) == 1))
    {
        ngx_rtmp_record_node_open(s, rctx);
    }
    /* add by ligp end */
