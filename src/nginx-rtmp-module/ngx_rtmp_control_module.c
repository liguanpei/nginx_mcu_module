...
#define NGX_RTMP_CONTROL_PULL_RECORD 0x0A
...
typedef struct {
    ngx_queue_t que;
    u_char 		stream_name[NGX_RTMP_MAX_NAME];
    u_char 		liveid[NGX_RTMP_MAX_ARGS];
} ngx_pull_record_stream_info;
...
typedef struct {
    ...
    ngx_queue_t                    *pull_streams; /* all pull record queue */
...
static ngx_conf_bitmask_t           ngx_rtmp_control_masks[] =   {
    ...
    { ngx_string("pull_record"),    NGX_RTMP_CONTROL_PULL_RECORD },
...
ngx_queue_insert1(ngx_queue_t *queue, ngx_walkstream_info *tmp_info) 
{
    ...
    void 
ngx_queue_insert2(ngx_queue_t *queue, ngx_pull_record_stream_info *tmp_info) 
{
    ngx_queue_init(&tmp_info->que);
    ngx_queue_insert_head(queue, &tmp_info->que);
}
...
ngx_print_queue1(ngx_queue_t *queue) 
{
    ...
    void 
ngx_print_queue2(ngx_queue_t *queue) 
{
    ngx_queue_t *q;
    ...
    for(q = ngx_queue_head(queue);
            ...
            q = ngx_queue_next(q))
    {
        ngx_pull_record_stream_info *eleNode = ngx_queue_data(q, ngx_pull_record_stream_info, que);
        printf("name = %s\n", eleNode->stream_name);
        printf("liveid = %s\n", eleNode->liveid);
    }
}
...
/* add by ligp end */
    ...
    static const char *
ngx_rtmp_control_pull_record_handler(ngx_http_request_t *r, ngx_rtmp_session_t *s)
{
    ngx_int_t                    rc;
    ngx_str_t                    rec;
    ngx_str_t                    liveid;
        ...
        ngx_rtmp_core_app_conf_t *cacf)
{
        ...
        for (n = 0; n < (ngx_uint_t) lacf->nbuckets; ++n) {
            for (ls = lacf->streams[n]; ls; ls = ls->next) {
                ...
                if (ngx_strlen(ls->name) != 0 && ls->publishing == 1) {
                    ...
                    ngx_pull_record_stream_info		*pullstream_info;

                    pullstream_info = (ngx_pull_record_stream_info *)ngx_pcalloc(
                            r->connection->pool, sizeof(ngx_pull_record_stream_info));

                    if (ngx_http_arg(r, (u_char *) "liveid", sizeof("liveid") - 1, &liveid) == NGX_OK) {
                        ngx_cpystrn(pullstream_info->liveid, liveid.data, liveid.len+1);
                    }

                    ngx_cpystrn(pullstream_info->stream_name, ls->name, ngx_strlen(ls->name) + 1);

                    if (ngx_find_queue1(ctx->pull_streams, pullstream_info->stream_name) == 0) {
                        ngx_queue_insert2(ctx->pull_streams, pullstream_info);
                        ngx_print_queue2(ctx->pull_streams);
                    }
                }
    ...
    if (0 == n) {
        ...
        pullstream_info = (ngx_pull_record_stream_info *)ngx_pcalloc(
                ngx_cycle->pool, sizeof(ngx_pull_record_stream_info));

        ngx_cpystrn(pullstream_info->stream_name, name.data, name.len + 1);
                ...
                ngx_strncmp(ctx->method.data, "start", ctx->method.len) == 0)
        {
            ...
            if (ngx_find_queue1(ctx->pull_streams, pullstream_info->stream_name) == 0) {
                ...
                rs = ngx_pcalloc(r->pool, sizeof(ngx_rtmp_relay_static_t));
                if (rs == NULL) {
                    return NGX_CONF_ERROR;
                }

                ngx_log_debug0(NGX_LOG_DEBUG_RTMP, r->connection->log, 0,
                        "control record: start pull");

                printf("rs = %p, %p\n", pull_rs, pull_rs->target);
                ctx = ngx_rtmp_relay_create_connection(&pull_rs->cctx, &name,
                        pull_rs->target);
                if (ctx) {
                    ctx->session->static_relay = 1; 
                    ngx_rtmp_relay_play_local(ctx->session);
                    printf("run play fun\n");
                    ...
                    ngx_cpystrn(vpublish.type, (u_char *)"live", 16);
                    ngx_rtmp_cmd_fill_args(vpublish.name, vpublish.args);

                    printf("111111111111111111\n");
                    ctx->session->app = ctx->app;
                    ngx_rtmp_exec_publish(ctx->session, &vpublish);
                    //if (ngx_rtmp_publish(s, &vpublish) != NGX_OK) {
                     //   return "publish failed";
                    //}
                }  
                ...
                if (liveid.len > 0) {
                    ngx_queue_set_liveid(ctx->pull_streams, pullstream_info->stream_name, pullstream_info->liveid);
                }
            }
        }
        else if (ctx->method.len == sizeof("stop") - 1 &&
                ngx_strncmp(ctx->method.data, "stop", ctx->method.len) == 0)
        {
            ...
            if (ngx_find_queue1(ctx->pull_streams, pullstream_info->stream_name) == 1) { 
                ngx_queue_remove_status(ctx->pull_streams, pullstream_info->stream_name);
            }
        }
    }
    /* add by ligp end */
...
error:
    ...
    static ngx_int_t
ngx_rtmp_control_pull_record(ngx_http_request_t *r, ngx_str_t *method)
{
    ngx_buf_t               *b;
    const char              *msg;
    ngx_chain_t              cl;
    ...
    msg = ngx_rtmp_control_walk(r, ngx_rtmp_control_pull_record_handler);
    if (msg != NGX_CONF_OK) {
        goto error;
    }
...
ngx_rtmp_control_handler(ngx_http_request_t *r)
{
    ...
    ctx->pull_streams = ngx_pcalloc(r->pool, sizeof(ngx_queue_t));
    ngx_queue_init(ctx->pull_streams);
...
#define NGX_RTMP_CONTROL_SECTION(flag, secname)                             \
    ...
    NGX_RTMP_CONTROL_SECTION(PULL_RECORD, pull_record);
