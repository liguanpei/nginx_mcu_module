...
ngx_rtmp_pull_record_t            *pull_rs = NULL;
...
ngx_rtmp_relay_push_reconnect(ngx_event_t *ev)
{
    ...
    for (n = 0; n < racf->pushes.nelts; ++n, ++t) {
		...
		if (ngx_queue_is_forbidden(ngx_rtmp_record_main_conf->queue, ctx->name.data, ctx->name.len) == 1 &&
								   ngx_strncmp(target->push_method.data, "push", 4) == 0) {
			ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
					     "[CCBoke Stream Info]: stream %V is forbidden to cdn for monitor when reconnect", &ctx->name);
			continue;
		}
...
{
    printf("ngx_rtmp_relay_create_connection\n");
    ngx_rtmp_relay_app_conf_t      *racf;
    ...
    if (ngx_parse_url(pool, &u) != NGX_OK) {
        if (u.err) {
            ngx_log_error(NGX_LOG_ERR, racf->log, 0,
                      "relay: resolve url(\"%V\") failed: %s", &u.url, u.err);
            goto clear;
        }
    }
...
#undef NGX_RTMP_RELAY_STR_COPY
    ...
    ngx_memcpy(pc->sockaddr, u.addrs->sockaddr, pc->socklen);
	...
	if (ngx_strncmp("cc_monitor_push", target->push_method.data, target->push_method.len) == 0) {
		rs->cc_monitor_push = 1;

	} else {
		rs->cc_monitor_push = 0;
	}
...
ngx_rtmp_relay_publish(ngx_rtmp_session_t *s, ngx_rtmp_publish_t *v)
{
    ...
    for (n = 0; n < racf->pushes.nelts; ++n, ++t) {
		...
		if (ngx_queue_is_forbidden(ngx_rtmp_record_main_conf->queue, name.data, name.len) == 1 && 
								   ngx_strncmp(target->push_method.data, "push", 4) == 0)
		{
			ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
					     "[CCBoke Stream Info]: stream %s is forbidden to cdn for monitor when connect", name.data);
			continue;
		}
...
ngx_rtmp_relay_app_conf_t          *pull_racf;
...
#undef NGX_RTMP_RELAY_NUM_PAR
    ...
    if (is_static) {
        ...
        *ee = e;
        ...
        rs->target = target;
        if (pull_rs == NULL) {
           pull_rs = ngx_pcalloc(cf->pool, sizeof(ngx_rtmp_pull_record_t));
           if (pull_rs == NULL) {
               return NGX_CONF_ERROR;
           }
           //pull_rs->target = target;
           pull_rs = rs;
        }   
    ...
    value = cf->args->elts;
	if (value[0].data[3] == 'l') {
		ngx_str_set(&target->push_method, "pull");
	...
	} else if (value[0].data[3] == 'h') {
		ngx_str_set(&target->push_method, "push");

	} else {
		ngx_str_set(&target->push_method, "cc_monitor_push");
	}
