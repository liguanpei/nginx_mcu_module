...
struct ngx_rtmp_live_ctx_s {
	...
	ngx_event_t                         delay_evt;
    unsigned                            active:1;
    unsigned                            publishing:1;
    unsigned                            publish_mothod:1;
    unsigned                            silent:1;
    unsigned                            paused:1;
	uint32_t                            s_time_counter;
};
...
typedef struct {
    ...
    ngx_msec_t                          buflen;
	ngx_int_t                           cc_avsync; // 音视频同步阈值
	ngx_int_t                           cc_delay_warning_value; // 流延迟告警阈值
	ngx_msec_t                          cc_delay_interval; // 延时探测周期
    ngx_pool_t                         *pool;
