        ...
        if (b->last - b->pos > n) {
            ...
            if (c >= (u_char)'0' && c <= (u_char)'9') {
                ...
                ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
                    		  "[CCBoke Stream Info]: notify: HTTP retcode: %dxx", (int)(c - '0'));

                if(c == '5' && b->pos[n+1] == (u_char)'8' && b->pos[n+2] == (u_char)'1') {

                    ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
                    		  "[CCBoke Stream Info]: notify: HTTP retcode: 581, room is monitoring");
                    return NGX_BUSY; // code 581, means this room is monitoring
                }
                ...
                switch (c) {
                    ...
                    case (u_char) '3':
                        return NGX_AGAIN;
					//case (u_char) '5': // code 581, means this room is monitoring
					//	return NGX_BUSY;
                    default: // code 4xx, means this room is forbidden
                        return NGX_ERROR;
                }
            }
        ...
        void *arg, ngx_chain_t *in)
{
    ...
    if (rc == NGX_ERROR) {
		...
		if (s->cc_monitor_push == 0) {
            ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
                    	  "[CCBoke Stream Info]: stream %s is forbidden for monitor", v->name);
			if (ngx_find_queue(ngx_rtmp_record_main_conf->queue, v->name) == 0)
			{
				ngx_stream_info *tmp_info;
				tmp_info = (ngx_stream_info *)ngx_pcalloc(ngx_cycle->pool, sizeof(ngx_stream_info));
				ngx_cpystrn(tmp_info->stream_name, v->name, ngx_strlen(v->name) + 1);
				tmp_info->is_record = 0;
				tmp_info->is_forbidden = 1;

				ngx_queue_insert(ngx_rtmp_record_main_conf->queue, tmp_info);
			} else {
				ngx_queue_set_monitor_info(ngx_rtmp_record_main_conf->queue, v->name, 1);
    ...
    if (rc == NGX_OK && ngx_find_queue(ngx_rtmp_record_main_conf->queue, v->name)) {
		ngx_queue_set_monitor_info(ngx_rtmp_record_main_conf->queue, v->name, 0);
