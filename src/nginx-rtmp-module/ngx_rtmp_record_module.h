...
typedef struct {
	ngx_queue_t    que;
	ngx_flag_t     is_record; // if record flag
	ngx_flag_t     is_forbidden; // if relay to cdn
...
typedef struct {
	ngx_queue_t   *queue;
} ngx_rtmp_record_main_conf_t;

ngx_rtmp_record_main_conf_t        *ngx_rtmp_record_main_conf;

ngx_int_t ngx_find_queue(ngx_queue_t *queue, u_char *name);
ngx_flag_t ngx_queue_need_record(ngx_queue_t *queue, u_char *name);
ngx_flag_t ngx_queue_is_forbidden(ngx_queue_t *queue, u_char *name, uint32_t len);
void ngx_queue_insert(ngx_queue_t *queue, ngx_stream_info *tmp_info);
void ngx_queue_set_monitor_info(ngx_queue_t *queue, u_char *name, ngx_flag_t forbitten);
void ngx_queue_set_record_info(ngx_queue_t *queue, u_char *name, ngx_flag_t record);
void ngx_queue_remove_status(ngx_queue_t *queue, u_char *name);
void ngx_queue_set_liveid(ngx_queue_t *queue, u_char *name, u_char *liveid);
void ngx_print_queue(ngx_queue_t *queue);
ngx_int_t ngx_queue_get_liveid(ngx_queue_t *queue, u_char *name, u_char *liveid);
/* add by ligp end */
