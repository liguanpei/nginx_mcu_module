...
static ngx_command_t  ngx_rtmp_live_commands[] = {
    ...
    { ngx_string("cc_avsync"), // 音视频同步告警阈值
      NGX_RTMP_MAIN_CONF|NGX_RTMP_SRV_CONF|NGX_RTMP_APP_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_num_slot,
      NGX_RTMP_APP_CONF_OFFSET,
      offsetof(ngx_rtmp_live_app_conf_t, cc_avsync),
      NULL },

    { ngx_string("cc_delay_warning_value"), // 流延迟告警阈值
      NGX_RTMP_MAIN_CONF|NGX_RTMP_SRV_CONF|NGX_RTMP_APP_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_num_slot,
      NGX_RTMP_APP_CONF_OFFSET,
      offsetof(ngx_rtmp_live_app_conf_t, cc_delay_warning_value),
      NULL },

    { ngx_string("cc_delay_interval"), // 延时时间监控周期
      NGX_RTMP_MAIN_CONF|NGX_RTMP_SRV_CONF|NGX_RTMP_APP_CONF|NGX_CONF_TAKE1,
      ngx_rtmp_live_set_msec_slot,
      NGX_RTMP_APP_CONF_OFFSET,
      offsetof(ngx_rtmp_live_app_conf_t, cc_delay_interval),
      NULL },
...
ngx_rtmp_live_create_app_conf(ngx_conf_t *cf)
{
    ...
    lacf->cc_delay_interval = NGX_CONF_UNSET_MSEC;
	lacf->cc_avsync = NGX_CONF_UNSET;
	lacf->cc_delay_warning_value = NGX_CONF_UNSET;
...
ngx_rtmp_live_merge_app_conf(ngx_conf_t *cf, void *parent, void *child)
{
	...
	ngx_conf_merge_value(conf->cc_avsync, prev->cc_avsync, 10000);
	ngx_conf_merge_value(conf->cc_delay_warning_value, prev->cc_delay_warning_value, 10000);
    ngx_conf_merge_msec_value(conf->cc_delay_interval, prev->cc_delay_interval, 10000);
...
static void
ngx_rtmp_compute_delay(ngx_event_t *ev)
{
    ngx_rtmp_live_app_conf_t       *lacf;
	...
	if (ctx->active && !ctx->delay_evt.timer_set) {
    	ngx_add_timer(ev, lacf->cc_delay_interval);
	}

	ctx->s_time_counter += lacf->cc_delay_interval;
	
	if (ctx->active) {
		ngx_int_t bw_in_video = ctx->stream->bw_in_video.bandwidth*8/1024;
		ngx_int_t bw_in_audio = ctx->stream->bw_in_audio.bandwidth*8/1024;

		if ((ngx_int_t)ctx->s_time_counter - (ngx_int_t)s->current_time >= lacf->cc_delay_warning_value) {
    		ngx_log_error(NGX_LOG_INFO, ctx->session->connection->log, 0,
					"[cclive_push_delay_overload]: bandwidth(video, audio): %dKb/s, %dKb/s, delay_tm: %dms, roomid=%s", 
					bw_in_video,
				  	bw_in_audio,
				  	ctx->s_time_counter - s->current_time,
				  	ctx->stream->name);
		}
		if (bw_in_video == 0) {
    		ngx_log_error(NGX_LOG_INFO, ctx->session->connection->log, 0,
					"[cclive_push_empty_bandwidth]: bandwidth(video, audio): %dKb/s, %dKb/s, delay_tm: %dms, roomid=%s", 
					bw_in_video,
				  	bw_in_audio,
				  	ctx->s_time_counter - s->current_time,
				  	ctx->stream->name);
    	...
    	ngx_log_error(NGX_LOG_INFO, ctx->session->connection->log, 0,
				"[CCBoke Stream Info]: Stream name: %s, bandwidth(video, audio): %dKb/s, %dKb/s, delay_tm: %dms", 
				ctx->stream->name,
				bw_in_video,
				bw_in_audio,
				ctx->s_time_counter - s->current_time);
                         ...
                         unsigned active)
{
    ...
    if (ctx->publishing) {
		...
		/* add cc delay time compute function by ligp */
		if (lacf->cc_delay_interval) {
			d_ev = &ctx->delay_evt;

			if (active && !ctx->delay_evt.timer_set) {
	        	d_ev->data = s;
	        	d_ev->log = s->connection->log;
				d_ev->handler = ngx_rtmp_compute_delay;

	    		ngx_add_timer(d_ev, lacf->cc_delay_interval);
				ctx->s_time_counter = 0;

			} else if(!active && ctx->delay_evt.timer_set) {
				ngx_del_timer(d_ev);
			}
		}
...
ngx_rtmp_live_close_stream(ngx_rtmp_session_t *s, ngx_rtmp_close_stream_t *v)
{
	...
	if (ctx->stream->publishing == 1 && ctx->publishing == 1) {
		ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
				      "[CCBoke Stream Info]: Stream stop publishing, name='%s'", ctx->stream->name);
	}
                 ...
                 ngx_chain_t *in)
{
	...
	av_sync = (ngx_int_t)ctx->cs[1].timestamp - (ngx_int_t)ctx->cs[0].timestamp;

	if (av_sync >= lacf->cc_avsync || av_sync <= -(lacf->cc_avsync)) {
		ngx_log_error(NGX_LOG_INFO, s->connection->log, 0, 
				      "[cclive_push_avsync_overload] avsync abnormal(v_ts: %d, a_ts: %d): %d, roomid=%s",
					  (ngx_int_t)ctx->cs[1].timestamp, (ngx_int_t)ctx->cs[0].timestamp, av_sync, ctx->stream->name);
	}
...
ngx_rtmp_live_publish(ngx_rtmp_session_t *s, ngx_rtmp_publish_t *v)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                   ...
                   v->name, v->type);
	// fix by ligp
	// if stream name is empty, we refuse it!
	if(ngx_strcmp(v->name, "") == 0)
	{
		ngx_log_debug(NGX_LOG_DEBUG_RTMP, s->connection->log, 0, "stream name is empty, we refuse it!");
    	ngx_log_debug2(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                   "live: publish: name='%s' args='%s'",
                   v->name, v->args);
		ngx_rtmp_send_status(s, "NetStream.Publish.BadName", "error", "Empty StreamName");
		return NGX_ERROR;
	}
    ...
    //ngx_rtmp_exec_init_process(ngx_cycle);
    ...
    if (!ctx->silent) {
        ngx_rtmp_send_status(s, "NetStream.Publish.Start",
                             "status", "Start publishing");
    	ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
					  "[CCBoke Stream Info]: Stream start publishing, name='%s', args='%s'", v->name, v->args);
    }
